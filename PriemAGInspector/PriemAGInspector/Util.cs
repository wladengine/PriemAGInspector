...
namespace PriemAGInspector
{
    static class Util
    {
        ...
        public static string DataFilesFolder { get; private set; }
        public static Form MainForm { get; set; }

        static Util()
        {
            //string connStr = "Data Source=81.89.183.103;Initial Catalog=OnlinePriem2012;Integrated Security=False;User ID=OnlinePriem2012Inspector;Password=372639BE-888B-4FF4-8D17-0E86B364566C;Connect Timeout=300";
            //string connStr = "Data Source=81.89.183.103;Initial Catalog=OnlinePriem2012;Integrated Security=False;User ID=tmp_tester;Password=1111111;Connect Timeout=300";
            string connStr = "Data Source=81.89.183.103;Initial Catalog=OnlinePriem2012;Integrated Security=True;Connect Timeout=300";
            //string connStr = "Data Source=(local);Initial Catalog=OnlinePriem2012;Integrated Security=True;Connect Timeout=300";
            
            BDC = new BDClass(connStr);
            TemplateFolder = Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments) + @"\PriemAGInspector_TempFiles\";
            DataFilesFolder = Application.StartupPath + @"\Data\";
            try
            {
        ...
        public static void OpenApplicationList()
        {
            ...
            List.MdiParent = MainForm;
            List.Show();
        }
       public static void OpenPersonCard(Form parent, string id, OnUpdateHandler OnUpdate)
        {
            foreach (Form f in MainForm.MdiChildren)
            {
                if (f is PersonCard)
            ...
            var pcard = new PersonCard(id);
            pcard.MdiParent = MainForm;
            pcard.OnUpdate += OnUpdate;
            pcard.Show();
        }
        public static void OpenAbiturientCard(Form parent, string id, Guid _PersonId)
        {
            foreach (Form f in MainForm.MdiChildren)
            {
            ...
            var abCard = new AbiturientCard(id, _PersonId);
            //parent.AddOwnedForm(abCard);
            abCard.MdiParent = MainForm;
            abCard.Show();
        }
        /*
        public static void OpenAbitsList()
        {
            foreach (Form f in MainForm.MdiChildren)
            {
                if (f is AbitsList)
            ...
            var AbList = new AbitsList();
            AbList.MdiParent = MainForm;
            AbList.Show();
        }
        
        public static void OpenEmailCard()
        {
            foreach (Form f in MainForm.MdiChildren)
            {
            }
        ...
        */
        ...
        public static void AddItems(this ComboBox cb, dynamic bindingSource)
        {
            if (bindingSource.Rows.Count == 0)
                return;
        ...
        public static void FindVal(this RadGridView rgv, string searchfield, string pattern)
        {
            int rwIndex = 0;
            foreach (GridViewRowInfo rw in rgv.Rows)
            {
                if (rgv.Rows[rw.Index].Cells[searchfield] != null 
                    && rgv.Rows[rw.Index].Cells[searchfield].Value.ToString().StartsWith(pattern, StringComparison.OrdinalIgnoreCase))
                {
                    if (!rgv.Rows[rw.Index].IsSelected)
                        rgv.ClearSelection();
                    rgv.CurrentRow = rgv.Rows[rw.Index];
                    return;
                }
                rwIndex++;
            }
        }
        public static void Email(string email, string text, string theme = "", string emailFrom = "no-reply@spbu.ru", string emailCopy = "")
        {
            System.Threading.Tasks.Task t = new System.Threading.Tasks.Task(_Email, new { To = email, Body = text, Subject = theme, From = emailFrom, Copy =  emailCopy });
            t.Start();
        }
        private static void _Email(dynamic data)
        {
            ...
            try
            {
                ...
                SRVPRIEM_MailerService.Service1Client client = new SRVPRIEM_MailerService.Service1Client();
        ...
        public static void Search(this DataGridView dgv, string columnName, string value)
        {
            foreach (DataGridViewRow row in dgv.Rows)
            {
                if (row.Cells[columnName].Value.ToString().StartsWith(value, StringComparison.OrdinalIgnoreCase))
                {
                    if (!(dgv.CurrentCell == dgv[columnName, row.Index]))
                    {
                        dgv.ClearSelection();
                        dgv.CurrentCell = dgv[columnName, row.Index];
                    }
                    else
                        dgv.CurrentCell = dgv[columnName, row.Index];
                    return;
        ...
        public static bool PersonHistory(Guid PersonId, Guid? AbitId, string sAction, string sOldValue, string sNewValue)
        {
            try
            {
                using (TransactionScope trans = new TransactionScope())
                {
                    string query = @"INSERT INTO PersonHistory (PersonId, AbiturientId, Action, OldValue, NewValue, Time, Owner) 
VALUES (@PersonId, @AbiturientId, @Action, @OldValue, @NewValue, @Time, @Owner)";
                    Dictionary<string, object> dic = new Dictionary<string, object>();
                    dic.Add("@PersonId", PersonId);
                    dic.AddVal("@AbiturientId", AbitId);
                    dic.Add("@Action", sAction);
                    dic.Add("@OldValue", sOldValue);
                    dic.Add("@NewValue", sNewValue);
                    dic.Add("@Time", DateTime.Now);
                    dic.Add("@Owner", System.Environment.UserName);

                    BDC.ExecuteQuery(query, dic);
