    ...
    public partial class PersonList : Telerik.WinControls.UI.RadRibbonForm
    {
        ...
        public string AppCount
        {
            get
            {
                try
                {
                    if (ddlAppCount.SelectedIndex == -1)
                        return ">0"; 
                }
                catch
                {
                    ddlAppCount.SelectedIndex = -1; 
                    return ">0";
                }
                return (string)ddlAppCount.Id();
            }
            set { ddlRegion.Id(value); }
        }
        public PersonList()
        ...
        private void FillComboAppCount()
        {
            Dictionary<object, string> bind = new Dictionary<object, string>();
            bind.Add(">0", "Любое количество");
            bind.Add("=1", "1 заявление");
            bind.Add("=2", "2 заявления");
            ddlAppCount.FillCombo(bind);
        }
        ...
        private void InitGrid()
        {
            ...
            def.ColumnGroups.Add(new GridViewColumnGroup("")); 
            def.ColumnGroups[5].ShowHeader = false;
            def.ColumnGroups[5].Rows.Add(new GridViewColumnGroupRow());
            def.ColumnGroups[5].Rows[0].Columns.Add(rgvList.Columns["AppCount"]);
        ...
        private void FillGrid()
        {
            ...
            string query = @"SELECT DISTINCT Person.Id, Surname + ' ' + Person.Name + ' ' + ISNULL(SecondName, '') AS FIO, 
                            ...
                            Person.Barcode, ";
            if (rdbTypeListApplications.ToggleState == Telerik.WinControls.Enumerations.ToggleState.On)
            {
                query += "  DateOfStart AS AppDate, ";
            }
            else if (rdbTypeListPersons.ToggleState == Telerik.WinControls.Enumerations.ToggleState.On)
            {
                query += " (SELECT MIN(DateOfStart) from AG_Application where PersonId=Person.Id and IsCommited=1 and Enabled=1) AS AppDate, ";
            }
            query += @" (SELECT MAX(LoadDate) from [AllFiles] AS AF WHERE AF.PersonId=Person.Id) AS LastAction,
    (select COUNT(id) from AG_Application where PersonId = Person.Id and Enabled = 1 and IsCommited = 1) as AppCount
                            FROM Person 
                            INNER JOIN [AG_Application] ON [AG_Application].PersonId = Person.Id 
                            ...
                            WHERE Person.RegistrationStage=@RegistrationStage and AG_Application.Enabled =1 and AG_Application.IsCommited=1";
            Dictionary<string, object> dic = new Dictionary<string, object>();
            ...
            {
                query += " AND Person.Id IN (SELECT Olympiads.PersonId FROM Olympiads) ";
            }
            ...
            if (chbApproved.Checked)
            {
               // query += " AND EXISTS (SELECT * FROM AG_Application WHERE PersonId=Person.Id AND IsApprovedByComission=1 AND Enabled=1 AND IsCommited=1)"; 
                query += " AND IsApprovedByComission=1"; 
            }
            query += " AND (select COUNT(id) from AG_Application where PersonId = Person.Id and Enabled = 1 and IsCommited = 1) " + AppCount;
            string sOrderBy = " ";
        ...
        private void rgvList_CellDoubleClick(object sender, GridViewCellEventArgs e)
        {
            if (e.RowIndex < 0)
                return;
            string Id = rgvList.Rows[e.RowIndex].Cells["Id"].Value.ToString();
        ...
        private void chbApproved_ToggleStateChanged(object sender, StateChangedEventArgs args)
        {
            FillGrid();
        }

        private void rdbTypeListApplications_Click(object sender, EventArgs e)
        {
            rdbTypeListApplications.IsChecked = !rdbTypeListPersons.IsChecked;
            FillGrid();
        }

        private void rdbTypeListPersons_Click(object sender, EventArgs e)
        {
            rdbTypeListPersons.IsChecked = !rdbTypeListApplications.IsChecked;
            FillGrid();
        } 

        private void ddlAppCount_SelectedIndexChanged(object sender, Telerik.WinControls.UI.Data.PositionChangedEventArgs e)
        {
            FillGrid();
        }
    }
